...
def extract():
    ...
    query1 = f"""
    SELECT to_date(current_timestamp()) insrt_on, COUNT(*) AS row_cnt
    FROM (
        ...
        WHERE DRILL_DATE = '{drill_date}'
          ...
          AND concat('D',DEPTID) in (
                select leaf from tleaves where treecd = 'DEPT' and node = 'ES100'
          )
          AND CONCAT('J',PROJECT_ID) IN (
                SELECT LEAF FROM TLEAVES where treecd = 'PROJ' and node = 'CRAG_CAPITAL_PROJ'
          )
        GROUP BY ACCOUNT, LEDGER, DRILL_DATE, PROJECT_ID, BUSINESS_UNIT, OPERATING_UNIT, LOCATION, DEPTID
        HAVING cast(sum(AMOUNT) AS DECIMAL(19,4)) != 0
    ) sub;
